...
class BVCDataFetcher:
    ...
    def get_ohlcv(
        ...
        Essaie Stooq en premier (pas de rate-limit), Yahoo Finance en fallback.
        """
        ...
        # ── 1. Stooq (fiable sur serveurs cloud, pas de rate-limit) ──────────
        df = _fetch_stooq(yahoo_ticker, period=period, start=start, end=end, interval=interval)

        # ── 2. Fallback Yahoo Finance avec retry exponentiel ──────────────────
        if df is None or df.empty:
            delays = [2, 6, 15]
